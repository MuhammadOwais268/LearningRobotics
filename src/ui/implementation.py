# File: client/src/ui/implementation.py (Final Version)
...
class ImplementationScreen(tk.Frame):
    ...
    # In client/src/ui/implementation.py

    def _populate_file_explorer(self):
        """
        Dynamically scans the Robotics/src and Robotics/lib directories
        and builds the Treeview based on the folders and files it finds.
        """
        # Clear any existing items from the tree first
        for i in self.file_tree.get_children():
            self.file_tree.delete(i)
        
        # Get the path to the permanent Robotics project
        ...
        if os.path.isdir(src_dir_path):
            for filename in sorted(os.listdir(src_dir_path)):
                # We only care about main.cpp for now
        ...
        # Iterate through each item in the 'lib' directory
        for library_folder_name in sorted(os.listdir(lib_dir_path)):
            library_path = os.path.join(lib_dir_path, library_folder_name)
            
            # Check if it's actually a directory (to ignore hidden files like .DS_Store)
            if os.path.isdir(library_path):
                # Insert the library folder name into the tree (e.g., 'Pololu_VL53L0X')
                lib_folder_id = self.file_tree.insert(lib_id, 'end', text=library_folder_name, open=True)
                
                # Now, iterate through the files inside that library folder
                for filename in sorted(os.listdir(library_path)):
                    # We only want to show .h and .cpp files
                    if filename.endswith('.h') or filename.endswith('.cpp'):
                        # This is the relative path we pass to the library viewer
    ...
    def load_content(self):
        ...
        if not self.is_in_edit_mode:
            ...
            if self.controller.current_user.get('role') == "developer": self.code_editor.config(state='disabled')
    ...
    def paste_into_widget(self, target_widget):
        if not self.is_in_edit_mode and target_widget != self.code_editor: return
        try:
            clipboard_content = self.clipboard_get()
            target_widget.delete(1.0, 'end'); target_widget.insert('end', clipboard_content)
